...
public class AvatarUtils {
     ...
     * @throws IOException
     */
    public static void generateAvatar(String name, String outputPath, String outputName) throws IOException {
        int width = 200;
        int height = 200;
        ...
        String filename = outputPath + File.separator + outputName;
        ...
        if (nameWritten.length() >= 2) {
            ...
            if (isChinese(firstWritten) && isChinese(secondWritten)) {
                g2.drawString(nameWritten, 40, 120);
            }
            //首中次英 如 罗Q
            else if (isChinese(firstWritten) && !isChinese(secondWritten)) {
                g2.drawString(nameWritten, 54, 120);
        ...
        if (nameWritten.length() == 1) {
            ...
            if (isChinese(nameWritten)) {
                ...
                g2.setFont(font);
                g2.drawString(nameWritten, 50, 140);
            }
            //英文
            else {
                font = new Font("微软雅黑", Font.PLAIN, 110);
                g2.setFont(font);
                g2.drawString(nameWritten.toUpperCase(), 66, 134);
            }

        }

        BufferedImage rounded = makeRoundedCorner(bi, 198);
