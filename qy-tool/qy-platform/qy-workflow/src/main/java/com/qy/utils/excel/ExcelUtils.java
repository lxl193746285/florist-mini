...
public class ExcelUtils {
    ...
    public static void export(HttpServletResponse response, String fileName, String sheetName,
        ...
        export(response, null, fileName, sheetName, sheetDataList, selectMap,cellTypeList);
    }

    private static void export(HttpServletResponse response, File file, String fileName, String sheetName,
                               List<List<Object>> sheetDataList, Map<Integer, List<String>> selectMap,List<Integer> cellTypeList) {
        // 整个 Excel 表格 book 对象
            ...
            for (int j = 0; j < rowList.size(); j++) {
                ...
                if (o instanceof URL) {
                    ...
                    if (i == 0) {
                        ...
                        Integer cellType = cellTypeList.get(j);
                        CellType newCellType = CellType.forInt(cellType);
                        v = setCellValue(cell, o, rowStyle,newCellType);
                    }
                }
                mergeArray[i][j] = v;
            }
    ...
    private static void export(HttpServletResponse response, File file, String fileName, String sheetName,
        ...
        for (int i = 0; i < sheetDataList.size(); i++) {
            ...
            List<Object> rowList = sheetDataList.get(i);
            for (int j = 0; j < rowList.size(); j++) {
                sheet.setDefaultColumnStyle(j,commonStyle);
                // 每个行数据中的单元格数据
                Object o = rowList.get(j);
                int v = 0;
                if (o instanceof URL) {
    ...
    private static int setCellValue(Cell cell, Object o, CellStyle style) {
        ...
        if (o instanceof String) {
            ...
            if (!StringUtils.isNullOfEmpty(s) && isNumeric(s)) {
     ...
     * @return
     */
    private static int setCellValue(Cell cell, Object o, CellStyle style,CellType cellType)
    {
        CellType newCellType=cellType;
        ...
        if (o instanceof String) {
            ...
            if (newCellType==null)
            {
                if (!Strings.isNullOrEmpty(s) && isNumeric(s)) {
                    cell.setCellType(CellType.NUMERIC);
                    cell.setCellValue(Double.parseDouble(s));
                    return CELL_OTHER;
                } else {
        ...
        {
            // 是否为字符串
            if (newCellType.equals(CellType.NUMERIC)) {
                cell.setCellType(CellType.NUMERIC);
                cell.setCellValue(Double.parseDouble(o.toString()));
                return CELL_OTHER;
            }
            // 是否为Boolean
            else if (newCellType.equals(CellType.BOOLEAN)) {
                cell.setCellType(CellType.BOOLEAN);
                cell.setCellValue((Boolean) o);
                return CELL_OTHER;
            }else if (newCellType.equals(CellType.STRING)) {
                // 如果是Date数据，则显示格式化数据
                if (o instanceof Date) {
                    cell.setCellType(CellType.STRING);
                    cell.setCellValue(formatDate((Date) o));
    ...
    public static <T, K> void theExport(HttpServletResponse response, String fileName, List<T> list, Class<K> template) {
        // list 是否为空
        boolean lisIsEmpty = list == null || list.isEmpty();
        ...
        // 如果模板数据为空，且导入的数据为空，则导出空文件
        if (template == null && lisIsEmpty) {
            theExportEmpty(response, fileName);
            return;
        }
        // 如果 list 数据为空，则导出模板数据
        if (lisIsEmpty) {
            theExportTemplate(response, fileName, template);
            return;
        }
        // 导出数据
        List<List<Object>> sheetDataList = getSheetData(list);
        theExport(response, fileName, sheetDataList);
    }
    public static <T> void theExportTemplate(HttpServletResponse response, String fileName, Class<T> clazz) {
        theExportTemplate(response, fileName, fileName, clazz, false);
    }
    public static <T> void theExportTemplate(HttpServletResponse response, String fileName, String sheetName,
                                          Class<T> clazz, boolean isContainExample) {
        // 获取表头字段
        List<ExcelClassField> headFieldList = getExcelClassFieldList(clazz);
        // 获取表头数据和示例数据
        ...
        // 导出数据
        theExport(response, fileName, sheetName, sheetDataList, selectMap);
    }
    public static void theExportEmpty(HttpServletResponse response, String fileName) {
        List<List<Object>> sheetDataList = new ArrayList<>();
        List<Object> headList = new ArrayList<>();
        headList.add("导出无数据");
        sheetDataList.add(headList);
    ...
    public static void theExport(HttpServletResponse response, String fileName, List<List<Object>> sheetDataList) {
        theExport(response, fileName, fileName, sheetDataList, null);
    }
    public static void theExport(HttpServletResponse response, String fileName, String sheetName,
                              List<List<Object>> sheetDataList, Map<Integer, List<String>> selectMap) {
        theExport(response, null, fileName, sheetName, sheetDataList, selectMap);
    }

    private static void theExport(HttpServletResponse response, File file, String fileName, String sheetName,
                               List<List<Object>> sheetDataList, Map<Integer, List<String>> selectMap) {
        String theSheetName ="";
        try{
        ...
        SXSSFWorkbook book = new SXSSFWorkbook();
        // 每个 Sheet 页
        Sheet sheet = book.createSheet(theSheetName);
        Drawing<?> patriarch = sheet.createDrawingPatriarch();
        // 设置表头背景色（灰色）
        CellStyle headStyle = book.createCellStyle();
        headStyle.setFillForegroundColor(IndexedColors.GREY_80_PERCENT.index);
    ...
    public static void exportWCH(HttpServletResponse response, File file, String fileName, String sheetName,
                               List<List<Object>> sheetDataList, Map<Integer, List<String>> selectMap) {
        try{
            sheetName= URLDecoder.decode(sheetName, "utf-8");
        }catch (UnsupportedEncodingException e){
    ...
    public static JSONArray readFileBySheetName(MultipartFile mFile, File file,String sheetName) throws Exception {
        ...
        JSONArray array = readSheet(sheet);
        book.close();
        in.close();
        return  array;
    }
