...
public class DataSourceInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        if (request.getRequestURI().contains("/v4/tool/interface/")){
            DynamicDataSourceContextHolder.push("platform");
            return true;
        }
        if (request.getRequestURI().contains("/v4/es/")){
            DynamicDataSourceContextHolder.push("platform");
            return true;
        }

        if (request.getRequestURI().contains("/v4/api/message") || request.getRequestURI().contains("/v4/message/")){
            DynamicDataSourceContextHolder.push("platform");
            return true;
        }
